...
public class DesR {
    ...
    String round(String input, String key, int num) {
        ...
        left = xor(left, temp);
        System.out.println(
                "Round " + (num + 1) + " " + right.toUpperCase() + " " + left.toUpperCase() + " " + key.toUpperCase());
        return right + left;
    }
    ...
    public static void main(String args[]) {
        try {
            ...
            String text = di.readUTF();
            String key = di.readUTF();
            String plainText = "";
            String ch = "";
            text = d.decrypt(text, key);
            for (int i = 0; i < text.length(); i = i + 2) {
                ch += text.charAt(i) + "" + text.charAt(i + 1);
                plainText += (char) Integer.parseInt(ch, 16);
                ch = "";
            }
